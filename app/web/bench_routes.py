...
from app.web.deps import BenchDep, is_htmx, page_context, templates
...
async def _own_duck(bench: Bench, duck_id: str) -> dict[str, str]:
    entry = await bench.entry(duck_id)
    ...
    return entry.duck.model_dump(include=set(_FIELD_LIMITS))
...
async def commission(
    ...
    try:
        ...
        return _form(request, values=values, errors=_field_errors(error), status_code=422)
    await bench.add_duck(draft)
    return _back_to_bench()
...
async def edit_duck(request: Request, bench: BenchDep, duck_id: str) -> HTMLResponse:
    return _form(request, values=await _own_duck(bench, duck_id), duck_id=duck_id)
...
async def amend(
    ...
    try:
        ...
        return _form(
            request, values=values, duck_id=duck_id, errors=_field_errors(error), status_code=422
        )
...
async def remove(request: Request, bench: BenchDep, duck_id: str) -> Response:
    error = None
    try:
